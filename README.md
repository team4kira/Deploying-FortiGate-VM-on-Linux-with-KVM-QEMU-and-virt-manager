...
[![Platform](https://img.shields.io/badge/Platform-Linux%20%7C%20KVM%2FQEMU-blue)](#requirements)
[![Virtualization](https://img.shields.io/badge/Manager-libvirt%20%7C%20virt--manager-orange)](#requirements)
[![FortiOS](https://img.shields.io/badge/Appliance-FortiGate--VM-red)](#download-the-image)
[![License](https://img.shields.io/badge/Use-Non--production%20lab-yellow)](#license-notes)
...
- [Prepare libvirt networking](#prepare-libvirt-networking)
- [Prepare the VM disk](#prepare-the-vm-disk)
- [Create the FortiGate VM](#create-the-fortigate-vm)
- [First boot and console setup](#first-boot-and-console-setup)
- [Open the web GUI](#open-the-web-gui)
- [Activate the evaluation license](#activate-the-evaluation-license)
- [Optional: isolated LAN](#optional-isolated-lan)
...
1. Sign in to the [Fortinet Support portal](https://support.fortinet.com/).
...
<details>
<summary><strong>Fedora / RHEL / Rocky Linux / AlmaLinux / CentOS Stream</strong></summary>
...
2. Confirm the connection is **QEMU/KVM - localhost**.
3. Select **Create a new virtual machine**.
4. Choose **Import existing disk image**.
5. Browse to and select `fortios.qcow2`.
6. Choose **Generic Linux** or the closest generic Linux version in the OS selector.
7. Allocate:
...
3. Do not choose **Upload License File** unless a paid VM license has been purchased.
4. Enter the FortiCare credentials used to sign in at [support.fortinet.com](https://support.fortinet.com/).
5. Submit the request.
6. Allow FortiGate to apply the license and reboot.
7. Wait for the console `login:` prompt to return, then refresh the browser and sign in again.
...
6. Select an **isolated** network with no forwarding to the physical network.
7. Disable libvirt DHCP if FortiGate will provide DHCP. Leave it enabled only for temporary testing if needed.
8. Complete the wizard and start the network.
